...
if __name__ == '__main__':
  ...
  if int(sys.argv[1])==0:
    ...
    ng=catalog.CatalogStore('ngmix_metacal_v1',cattype='ng',cols=cols,catdir='/share/des/disc3/ngmix/',release='y1',p=p,cutfunc=catalog.CatalogMethods.final_null_cuts_id(),hdu=1,maxrows=50000000)
...
# for i in xrange(10):
#   for j in xrange(10):
#     chi2+=((gp[0,i])*(gp[0,j])/np.load('covgp.npy')[i,j])
...
#     if j!=i:
#       continue
#     chi2+=((gp[0,i]-0.00011817854895108219*((r[1:]+r[:-1])[i]/2./20.)**-1.7421121804218127)*(gp[0,j]-0.00011817854895108219*((r[1:]+r[:-1])[j]/2./20.)**-1.7421121804218127)/np.load('covgp.npy')[i,j])
...
#   def func(x,m,b):
#     return m*(x/20.)**b
#   params=curve_fit(func,x,y,p0=(0.,-.73),sigma=sig)
#   m,b=params[0]
...
#   def func(x,m):
#     return m*(x/20.)**-.73
#   params=curve_fit(func,x,y,p0=(0.),sigma=sig)
#   m=params[0]
#   merr=np.sqrt(np.diagonal(params[1]))
#   return m,merr


  # fig.plot_methods.plot_IA((r[1:]+r[:-1])/2.,[gp[0],gx[0]],[gperr,gxerr],'ge')
  ...
  cols=['stamp','nexp','chi2pix','psffwhm','coadd','info','like','flux','rgp','dec','evals','rad','dec_off','ra_off','dflux','fluxfrac','psf1','psf2','hsmpsf1','hsmpsf2','modmax','modmin','ra','resmax','tile','maskfrac','snr','resmin','e1','e2','iter','bflux','dflux','flux','cov11','cov22','zp',]
  cols=['coadd','info','rgp','dec','ra','snr','e1','e2','psf1','psf2','psffwhm','nexp','like','rad','flux']
  cols=['coadd','info','dec','ra','e1','e2','tile']
  ...
  i3=catalog.CatalogStore('y1_i3_sv_v1',cattype='i3',cols=cols,catdir='/share/des/disc2/y1/im3shape/single_band/r/y1v1/complete/main/',release='y1')
  sim=catalog.CatalogStore('y1_i3_sv_v1',cattype='i3',cols=cols,catdir='/share/des/disc2/image-sims/simulation_data/results/',release='y1')
...
labels=np.zeros(len(rmdens.coadd))
...
for i in range(50):
  ...
  labelsspt=kmspt.find_nearest(np.vstack((tmp['RA'][tmp['DEC']<-10],tmp['DEC'][tmp['DEC']<-10])).T)
  tmplabel[tmp['DEC']<-10]=labelsspt
  tmplabel[tmp['DEC']>-10]=labelss82+100
  if i==0:
    labels=tmplabel
  else:
...
for i in range(50):
  ...
  if i==0:
    ...
    labels=np.append(labels,tmplabel)
np.save(config.redmagicdir+'highlum_ran_regs.npy',labels)


spec=fio.FITS('spec_gold_catalog_y1a1_weights.fits')[-1].read()
spec['RA'][spec['RA']>180]=spec['RA'][spec['RA']>180]-360
#y1mask=np.in1d(spec['TILENAME'],np.unique(i3.tile),assume_unique=False)
i3mask=np.random.choice(np.arange(len(i3.coadd)),100000,replace=False)
shapes2=np.in1d(spec['COADD_OBJECTS_ID'],i3.coadd[i3.info==0],assume_unique=True)
...
for i,x in enumerate(np.unique(spec['source'])):
  mask=(spec['source']==x)
  print i,x,np.sum(mask),np.sum(shapes&mask),np.sum(shapes2&mask)
  if np.sum(mask)==0:
    continue
  print 'min',np.min(spec['RA'][mask]),np.min(spec['DEC'][mask])
  print 'max',np.max(spec['RA'][mask]),np.max(spec['DEC'][mask])
  plt.plot(spec['RA'][mask],spec['DEC'][mask],marker=',',linestyle='')
plt.savefig('tmp.png')
plt.close()
...
# 0 2df
# 6 DEEP2 http://deep.ps.uci.edu/DR4/completeness.html
# 8 DR12_CMASS 
# 9 DR12_LOWZ 
# 18 SDSS_DR7 
# 25 VIPERS fornax
...
# 5 CDB           66 66 0
# min 36.043362 -4.862245
# max 150.570971 2.67502
# 6 DEEP2         7161 7160 2560
# min -8.629864 -0.102582
# max -6.388147 0.434746
# 7 DES_AAOMEGA   12436 11837 1631
# min 6.453162 -44.967538
# max 55.431726 0.960018
# 8 DR12_CMASS    16857 16727 8767
# min -42.51883 -1.76792
# max 43.833122 2.510662
# 9 DR12_LOWZ     5222 4471 1541
# min -42.518471 -1.767586
# max 43.379613 2.509192
# 10 EBOSS_DES_ELG 4432 4281 0
# min 33.567073 -7.435126
# max 37.517577 -3.628078
...
# 20 SNLS_AAO      98 95 0
# min 35.989664 -4.988209
# max 36.943488 -3.993808
# 21 SNLS_FORS     818 795 0
# min 35.998805 -5.01484
# max 150.605608 2.716263
# 22 SPARCS        197 196 78
# min 8.620546 -44.231602
# max 9.2486 -43.075319
# 23 STALIN        63 61 0
# min 35.05348 -5.344455
...
# max -0.707586 5.823688
# 29 XXL_AAOMEGA   2491 1472 492
# min -11.887299 -56.710227
# max -2.226731 -52.124983
# 30 ZCOSMOS       11751 10829 0
# min 149.456716 1.608869
# max 150.773155 2.850941


for i in range(200):
  if i==0:
    tmp=np.genfromtxt('/share/des/disc2/image-sims/simulation_data/results/FLUXTEST23295['+str(i)+'].fornax.jb.man.ac.uk.main.txt',names=True)
  else:
    tmp=np.append(tmp,np.genfromtxt('/share/des/disc2/image-sims/simulation_data/results/FLUXTEST23295['+str(i)+'].fornax.jb.man.ac.uk.main.txt',names=True))

x,y=catalog.CatalogMethods.sort2(i3.coadd,sim.coadd)
...
plt.xlabel(s)
plt.minorticks_on()
plt.savefig('plots/hist/hist_'+i3.name+'_'+sim.name+'_'+'ra'+'.png', bbox_inches='tight')
plt.close()

plt.figure()
plt.hist(i3.dec[x],bins=100,alpha=.5)
plt.hist(sim.dec[y],bins=100,alpha=.5)
plt.ylabel(r'$n$')
s=config.lbl.get('dec',None)
if config.log_val.get('dec',None):
  s='log '+s

plt.xlabel(s)
plt.minorticks_on()
plt.savefig('plots/hist/hist_'+i3.name+'_'+sim.name+'_'+'dec'+'.png', bbox_inches='tight')
plt.close()
...
plt.hist(sim.psf1[y],bins=100,range=(-.1,.1),alpha=.5)
plt.ylabel(r'$n$')
s=config.lbl.get('psf1',None)
if config.log_val.get('psf1',None):
  s='log '+s

plt.xlabel(s)
plt.minorticks_on()
plt.savefig('plots/hist/hist_'+i3.name+'_'+sim.name+'_'+'psf1'+'.png', bbox_inches='tight')
plt.close()

plt.figure()
plt.hist(i3.psf2[x],bins=100,range=(-.1,.1),alpha=.5)
plt.hist(sim.psf2[y],bins=100,range=(-.1,.1),alpha=.5)
plt.ylabel(r'$n$')
s=config.lbl.get('psf2',None)
if config.log_val.get('psf2',None):
  s='log '+s

plt.xlabel(s)
plt.minorticks_on()
plt.savefig('plots/hist/hist_'+i3.name+'_'+sim.name+'_'+'psf2'+'.png', bbox_inches='tight')
plt.close()
...
nominal=desmag[mask]-usd[match[1].reshape(len(match[1]))[mask]]-3.5+apass1[mask]

final=desmag-np.mean(nominal[~np.isinf(nominal)&~np.isnan(nominal)])+3.5-apass1

image2=np.ones(data0.shape, dtype=data0.dtype.descr + [('expnum',int)] + [('apass','f8')] + [('zp','f8')] + [('mag_auto_u','f8')] + [('magerr_auto_u','f8')])

for name in data0.dtype.names:
  image2[name]=data0[name]

image2['expnum']=exp1
image2['apass']=apass1
image2['zp']=-np.mean(nominal[~np.isinf(nominal)&~np.isnan(nominal)])+3.5-apass1
image2['mag_auto_u']=-2.5*np.log10(data0['flux_auto'])-np.mean(nominal[~np.isinf(nominal)&~np.isnan(nominal)])+3.5-apass1
image2['magerr_auto_u']=2.5*np.log10(data0['fluxerr_auto']/data0['flux_auto']+1.)
...
cols=[ 'chi2pix', 'psffwhm', 'like', 'rgp', 'dec', 'evals', 'rad', 'dec_off', 'ra_off', 'fluxfrac', 'psf1', 'psf2', 'hsmpsf1', 'hsmpsf2', 'modmax', 'modmin', 'ra', 'resmax',  'maskfrac', 'snr', 'resmin', 'e1', 'e2', 'flux', 'g', 'r', 'i', 'z', 'bfrac']
sys_split.split.cat_splits_lin_e(i3,cols=['psf1'])
for x in cols:
  edge=lin.linear_methods.find_bin_edges(getattr(i3,x),3,w=None)
  for i in range(3):
    print x,i
    mask=(getattr(i3,x)>edge[i])&(getattr(i3,x)<=edge[i+1])
    sys_split.split.cat_splits_lin_e(i3,cols=['psf1'],mask=mask)

for i in range(4):
  print 'nexp',i
  mask=(getattr(i3,'nexp')==i+1)
  if np.sum(mask)>0:
    sys_split.split.cat_splits_lin_e(i3,cols=['psf1'],mask=mask)

cols=['rgp']
for x in cols:
  edge=lin.linear_methods.find_bin_edges(getattr(i3,x),7,w=None)
  for i in range(7):
    print x,i
    mask=(getattr(i3,x)>edge[i])&(getattr(i3,x)<=edge[i+1])
    sys_split.split.cat_splits_lin_e(i3,cols=['psf1'],mask=mask)
...
for x in cols:
  edge=lin.linear_methods.find_bin_edges(i3.ra*i3.dec,20,w=None)
  for i in range(20):
    print x,i
    mask=(i3.ra*i3.dec>edge[i])&(i3.ra*i3.dec<=edge[i+1])
    sys_split.split.cat_splits_lin_e(i3,cols=['psf1'],mask=mask)

for i in range(20):
  mask=(i3.ra*i3.dec>edge[i])&(i3.ra*i3.dec<=edge[i+1])
  plt.plot(i3.ra[mask],i3.dec[mask],ls='',marker='.')
...
for i in range(3):
  plt.plot(np.loadtxt('nzs/BCCUFIG_SV_bpz_nouband_zmean_'+bounds[i]+'_'+bounds[i+1]+'_stackedpz.txt')[:,0],np.loadtxt('nzs/BCCUFIG_SV_bpz_nouband_zmean_'+bounds[i]+'_'+bounds[i+1]+'_stackedpz.txt')[:,1],color='r')
  plt.plot(np.loadtxt('nzs/BCCUFIG_SV_bpz_nouband_zmean_'+bounds[i]+'_'+bounds[i+1]+'_truezhistogram.txt')[:,0],np.loadtxt('nzs/BCCUFIG_SV_bpz_nouband_zmean_'+bounds[i]+'_'+bounds[i+1]+'_truezhistogram.txt')[:,1],color='k')
  plt.axvline(x=np.average(np.loadtxt('nzs/BCCUFIG_SV_bpz_nouband_zmean_'+bounds[i]+'_'+bounds[i+1]+'_stackedpz.txt')[:,0],weights=np.loadtxt('nzs/BCCUFIG_SV_bpz_nouband_zmean_'+bounds[i]+'_'+bounds[i+1]+'_stackedpz.txt')[:,1]), ymin=0., ymax = 1, linewidth=1, color='r')
  plt.axvline(x=np.average(np.loadtxt('nzs/BCCUFIG_SV_bpz_nouband_zmean_'+bounds[i]+'_'+bounds[i+1]+'_truezhistogram.txt')[:,0],weights=np.loadtxt('nzs/BCCUFIG_SV_bpz_nouband_zmean_'+bounds[i]+'_'+bounds[i+1]+'_truezhistogram.txt')[:,1]), ymin=0., ymax = 1, linewidth=1, color='k')

plt.savefig('nou.png')
plt.close()

for i in range(3):
  plt.plot(np.loadtxt('nzs/BCCUFIG_SV_bpz_withuband_zmean_'+bounds[i]+'_'+bounds[i+1]+'_stackedpz.txt')[:,0],np.loadtxt('nzs/BCCUFIG_SV_bpz_withuband_zmean_'+bounds[i]+'_'+bounds[i+1]+'_stackedpz.txt')[:,1],color='b')
  plt.plot(np.loadtxt('nzs/BCCUFIG_SV_bpz_withuband_zmean_'+bounds[i]+'_'+bounds[i+1]+'_truezhistogram.txt')[:,0],np.loadtxt('nzs/BCCUFIG_SV_bpz_withuband_zmean_'+bounds[i]+'_'+bounds[i+1]+'_truezhistogram.txt')[:,1],color='k')
...
for i in range(5):
  ...
  plt.plot(np.loadtxt('nzs/BCCUFIG_SV_bpz_nouband_zmean_'+bounds1[i]+'_'+bounds2[i]+'_truezhistogram.txt')[:,0],np.loadtxt('nzs/BCCUFIG_SV_bpz_nouband_zmean_'+bounds1[i]+'_'+bounds2[i]+'_truezhistogram.txt')[:,1],color='k')
...
pz0=catalog.PZStore('bpzu',setup=False,pztype='dnf',filetype='h5',file=config.pzdir+'dnf_validation.hdf5')
pz0.name='bpzu'
pz0.pztype='bpzu'
pz0.pzsource=np.zeros((6,199))
pz0.pzlens=np.zeros((4,100))
pz0.specsource=np.zeros((6,30))
pz0.bins=np.loadtxt('nzs/BCCUFIG_SV_bpz_withuband_zmean_0.3_1.2_stackedpz.txt')[:,0]
pz0.binspec=np.loadtxt('nzs/BCCUFIG_SV_bpz_withuband_zmean_0.3_1.2_truezhistogram.txt')[:,0]
pz0.binl=np.loadtxt('/home/troxel/des-mpp/zdistris/zdistribution_redm')[:,0]
pz0.pzsource[0,:]=np.loadtxt('nzs/BCCUFIG_SV_bpz_withuband_zmean_0.3_1.2_stackedpz.txt')[:,1]
pz0.specsource[0,:]=np.loadtxt('nzs/BCCUFIG_SV_bpz_withuband_zmean_0.3_1.2_truezhistogram.txt')[:,1]
bounds1=['0.3','0.5','0.7','0.8','1']
bounds2=['0.48','0.66','0.84','1','1.2']
for i in range(5):
  pz0.pzsource[i+1,:]=np.loadtxt('nzs/BCCUFIG_SV_bpz_withuband_zmean_'+bounds1[i]+'_'+bounds2[i]+'_stackedpz.txt')[:,1]
  pz0.specsource[i+1,:]=np.loadtxt('nzs/BCCUFIG_SV_bpz_withuband_zmean_'+bounds1[i]+'_'+bounds2[i]+'_truezhistogram.txt')[:,1]

pz0.pzlens[0,:]=np.mean(np.loadtxt('/home/troxel/des-mpp/zdistris/zdistribution_redm')[:,1:],axis=1)
for i in range(3):
  pz0.pzlens[i+1,:]=np.loadtxt('/home/troxel/des-mpp/zdistris/zdistribution_redm')[:,i+1]


cosmo.make.fits(pz0.binl,pz0.pzlens[0,:],pz0.bins,pz0.pzsource[0,:],config.pztestdir+test+'/nofz/notomo_'+pz0.name+'.fits.gz')
...
pz1.specsource=np.zeros((6,30))
pz1.bins=np.loadtxt('nzs/BCCUFIG_SV_bpz_withuband_zmean_0.3_1.2_stackedpz.txt')[:,0]
