...
class y1_plots(object):
    ...
    def mean_e_subplot(cat,n,val,fig,replace=False,psf=None):
        ...
        if val == 'psf1':
            plt.errorbar(np.concatenate(([xmin],d['arr1'],[xmax])),d['m']*np.concatenate(([xmin],d['arr2'],[xmax]))+d['b1'],marker='',linestyle=':',color='r',lw=4)
        ...
        if val == 'psf2':
            plt.errorbar(np.concatenate(([xmin],d['arr1'],[xmax])),d['m']*np.concatenate(([xmin],d['arr2'],[xmax]))+d['b2'],marker='',linestyle=':',color='b',lw=4)
        ax.minorticks_on()
